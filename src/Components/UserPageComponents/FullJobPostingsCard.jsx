  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:8086/jobs/getnumber", {
        vendorName: company, // ✅ Backend expects "vendorName", not "companyName"
      });
    ...
    <div className="Full-Job-Postings_card">
      ...
      <main className="Full-Job-job-container">
        <section className="Full-Job-job-details">
          ...
          <p>
            {company || "Unknown Company"} <span className="Full-Job-rating">⭐ {rating}</span>
          </p>
        ...
        <section className="Full-Job-overview">
          <h3>Overview</h3>
        ...
        <section className="Full-Job-job-description">
          ...
          {description.length > 0 ? description.map((desc, index) => (
            <div key={index}>{formatText(desc)}</div>
        ...
        <section className="Full-Job-Accommodation">
          <h3>Accommodation</h3>
          <ul>
            {accommodations.length > 0 ? accommodations.map((acc, index) => (
              <li key={index}>
                <strong>{acc.year}</strong> - <div>{formatText(acc.detail)}</div>
              </li>
        ...
        <section className="Full-Job-beware-imposters">
          ...
          <button className="Full-Job-Report-Button">Report</button>
        </section>
      </main>
    </div>
  );
